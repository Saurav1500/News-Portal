...
@section('title', 'क्रल स्रोत सम्पादन - News AI')
@section('content')
<div class="dashboard-content">
    <div class="page-header">
        <div class="header-top">
            <div>
                <h1 class="page-title">क्रल स्रोत सम्पादन</h1>
                <p class="page-subtitle">{{ $crawlSource->name }}</p>
            </div>
    ...
    <div class="form-card">
        <form action="{{ route('dashboard.crawl-sources.update', $crawlSource) }}" method="POST">
            @csrf @method('PUT')
            <div class="form-grid">
                <div class="form-group">
                    <label for="name">स्रोतको नाम <span class="required">*</span></label>
                    <input type="text" id="name" name="name" required value="{{ old('name', $crawlSource->name) }}">
                    @error('name') <span class="field-error">{{ $message }}</span> @enderror
                </div>
                <div class="form-group">
                    <label for="url">URL <span class="required">*</span></label>
                    <input type="url" id="url" name="url" required value="{{ old('url', $crawlSource->url) }}">
                ...
                <div class="form-group">
                    ...
                    <select id="category_id" name="category_id">
                        ...
                        @foreach($categories as $cat)
                            <option value="{{ $cat->id }}" {{ old('category_id', $crawlSource->category_id) == $cat->id ? 'selected' : '' }}>{{ $cat->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    <label for="crawl_interval_minutes">क्रल अन्तराल (मिनेट)</label>
                    <input type="number" id="crawl_interval_minutes" name="crawl_interval_minutes" value="{{ old('crawl_interval_minutes', $crawlSource->crawl_interval_minutes) }}" min="5">
                </div>
                <div class="form-group">
                    <label>
                        <input type="checkbox" name="is_active" value="1" {{ $crawlSource->is_active ? 'checked' : '' }}>
                        सक्रिय
                    </label>
                </div>
